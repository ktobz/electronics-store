...
import { initiateGoogleLogin, promptGoogleEmail } from '../services/googleAuth';
...
const SignupPage: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    setLoading(true);
    setError(null);
    const mode = await initiateGoogleLogin();
    if (mode === 'email') {
  ...
  const handleEmailSubmit = () => {
    promptGoogleEmail(googleEmail, () => {}, (msg) => setError(msg));
  };
        ...
        <motion.div className="auth-card" initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5 }}>
          ...
          {showEmailPrompt ? (
            <div className="auth-form" style={{ textAlign: 'center' }}>
              <p style={{ marginBottom: '16px', color: '#555' }}>Enter your Google email to continue</p>
              <div className="form-group">
                <div className="input-wrapper">
                  <Mail size={20} className="input-icon" />
                  <input type="email" value={googleEmail} onChange={e => setGoogleEmail(e.target.value)} placeholder="you@gmail.com" required />
                </div>
              </div>
              {error && (
              ...
              <motion.button type="button" className="auth-button" onClick={handleEmailSubmit} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                Continue
              </motion.button>
            </div>
          ) : (
